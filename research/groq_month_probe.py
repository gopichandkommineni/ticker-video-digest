...
This is the Groq counterpart of `gemini_month_probe.py`. Same four stages,
same strict-JSON contracts, same resumable ledger — different provider, so the
...
Stages:
  ...
  3. Sector grouping (LLM)      — ONE Groq call maps unique tickers -> sector.
...
BATCH_SIZE = 25                # Stage 4 tweets per Groq call (batched)
MAX_THESIS_BATCHES = 60        # safety cap (Groq RPD is generous; backstop only)
MAX_LLM_CALLS = 100            # daily call budget guard (Groq free tier is roomy)
BATCH_TEXT_TRUNCATE = 300      # per-tweet chars sent inside a batch (smaller = faster)
DELAY_SECONDS = 2.1            # politeness: keep under ~30 req/min on free tier
REQUEST_TIMEOUT = 150          # large batched responses are slow; allow headroom
MAX_429_RETRIES = int(os.environ.get("GROQ_MAX_429_RETRIES", "4"))  # per-call: wait out the TPM bucket and retry
RATE_LIMIT_WAIT = 30.0         # fallback wait (s) when no reset header is given
RATE_LIMIT_WAIT_CAP = 70.0     # never sleep longer than this on a single 429
...
REPORT_DIR = Path(__file__).parent / "probes" / "groq_digest" / f"{RUN_DATE}_30d"
...
Return STRICT JSON: an object with a single key "results" whose value is an
array of the SAME length, one object per input tweet, echoing each "id". Each
...
def _parse_reset(s: str | None) -> float | None:
    """Parse a Groq reset duration like '21.36s', '1m30s', '2m' into seconds."""
    if not s:
        return None
    total = 0.0
    matched = False
    for value, unit in re.findall(r"([\d.]+)\s*(ms|s|m|h)", s):
        matched = True
        v = float(value)
...
def groq_json(prompt: str, api_key: str) -> dict:
    """Single Groq call returning parsed JSON.

    The Groq free tier is bound by a tokens-per-minute limit (not a daily
    request cap), so 429s are transient — the budget refills within a minute.
    On 429 this honours the `retry-after` / `x-ratelimit-reset-tokens` header
    and retries a bounded number of times. Raises RateLimited only if the
    budget never recovers within the retry window.
    """
    payload = {
        "model": GROQ_MODEL,
        "temperature": 0.0,
    ...
    for attempt in range(MAX_429_RETRIES + 1):
        ...
        try:
            ...
            if e.code != 429:
                raise
            last_detail = e.read().decode("utf-8", "replace")[:200]
            if attempt >= MAX_429_RETRIES:
                break
            # Wait for the token bucket to refill, then retry.
            wait = _parse_reset(
                e.headers.get("retry-after")
                or e.headers.get("x-ratelimit-reset-tokens")
            )
            wait = min(max(wait or RATE_LIMIT_WAIT, 1.0), RATE_LIMIT_WAIT_CAP)
            print(f"    429 — waiting {wait:.0f}s for token budget "
...
def main() -> int:
    ...
    R(f"- Model: `{GROQ_MODEL}` (Stages 3 & 4 only)")
    R()
    ...
    # attempt (incl. retries) and enforces the daily budget backstop.
    succeeded = 0
    rate_limited = 0
    llm_calls = 0
    ...
    R()
    R("> ⚠️ **Probe-only.** Sectors below are assigned by a single Groq call")
    ...
    if not api_key:
        ...
        R("_Sector map loaded from cache (`sectors.json`); no Groq call spent._")
    elif unique_tickers:
        prompt = SECTOR_PROMPT.format(tickers=", ".join(unique_tickers))
        try:
            llm_calls += 1
            raw = groq_json(prompt, api_key)
            succeeded += 1
    ...
    R(f"Tweets are batched **{BATCH_SIZE} per Groq call** to stay within the")
    R("free-tier tokens-per-minute limit while covering the full month.")
    R()
    ...
    if not api_key:
        ...
        def run_batch(bi: int, batch: list[dict], label: str) -> bool:
            ...
            try:
                llm_calls += 1
                parsed = groq_json(prompt, api_key)
                succeeded += 1
    ...
    R("## Run summary")
    R()
    R(f"- Groq calls attempted: **{llm_calls}** (daily budget guard: "
      f"{MAX_LLM_CALLS})")
    R(f"- Groq calls succeeded: **{succeeded}**")
    R(f"- Groq calls rate-limited (429): **{rate_limited}**")
