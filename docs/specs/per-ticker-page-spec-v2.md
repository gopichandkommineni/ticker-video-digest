# Per-Ticker Page — Design Specification v2.0

**Project:** Casino-Coherent Momentum Dashboard
**Date locked:** 2026-05-10
**Status:** v2 adds embedded TradingView Technical Analysis widget. v1 retained for history.
**Supersedes:** docs/specs/per-ticker-page-spec-v1.md
...
- The TradingView embedded widget provides chart-level technical data without adding our own charting overhead
...
### Technical Analysis — TradingView widget

A full-width embedded TradingView Technical Analysis widget, positioned **below Tier 3 tiles and above the Fundamentals strip**.

- **Section heading:** "Technical Analysis"
- **Subtitle:** "Across 27+ indicators and 11 timeframes, via TradingView"
- **Widget:** TradingView's official free embeddable Technical Analysis widget
  - Symbol: dynamically resolved from current ticker (format: `"NASDAQ:AAOI"`, `"NYSE:GLW"`, etc.)
  - Display mode: `"single"` (just the gauge)
  - Interval: `"1D"` (daily), with interval tabs visible
  - Theme: `"light"` (dark mode detection deferred to follow-up)
  - Width: 100% of container
  - Height: ~450px (TradingView's recommended for single mode)
- **Attribution line** (below widget): small italic gray text — *"Updates in real-time during market hours. Source: TradingView."*
- **Exchange prefix handling:** Resolved via static mapping in `src/casino_dashboard/ui/components/tradingview.py`. Falls back to `NASDAQ:TICKER` for unmapped symbols.
- **Theme detection note (v2):** Defaults to `"light"`. A follow-up can detect via `st.context` or similar; not worth blocking on now.

---

### Fundamentals strip — wide reference section

A horizontal strip below the Technical Analysis widget, before the news. Visually distinct from tiles (no rounded corners, lighter background, smaller text). Contains 5 reference values:
...
├────────────────────────────────────────────────────────┤
│ TECHNICAL ANALYSIS                                     │
│ Across 27+ indicators and 11 timeframes, via TradingView│
│ ┌──────────────────────────────────────────────────┐   │
│ │  [TradingView Technical Analysis Widget]         │   │
│ │   Buy / Neutral / Sell gauge                     │   │
│ │   NASDAQ:AAOI · 1D                               │   │
│ └──────────────────────────────────────────────────┘   │
│ Updates in real-time during market hours. Source: TV.  │
├────────────────────────────────────────────────────────┤
│ FUNDAMENTALS                                           │
...
12. Ownership
13. Red Flag
14. Technical Analysis (TradingView widget — responsive, 100% width)
15. Fundamentals (4×2 grid, two columns)
16. News (collapsed by default with "Show news" toggle)
17. Footer
...
- ❌ Our own price chart / candlesticks / sparklines (TradingView widget covers chart-level technical data via the embedded gauge)
- ❌ Technical indicators on chart (covered by TradingView widget gauge; Finviz linkout for more)
...
- **Dark mode**: if implemented, what's the dark equivalent palette? (TradingView widget theme should switch to "dark" to match)
...
1. **Commit this document** to your repo as `docs/specs/per-ticker-page-spec-v2.md`
...
- **v2 adds embedded TradingView Technical Analysis widget; revises v1's no-charts decision.** The widget is a third-party embed (no charting code owned by us), positioned between Tier 3 tiles and Fundamentals strip. Theme defaults to "light" in v2; dark-mode follow-up work deferred.
